...
func NastashaChannelerOfSuns(c *match.Card) {
	...
	c.Use(fx.Creature, fx.When(fx.BreakShield, fx.Doublebreaker, func(card *match.Card, ctx *match.Context) {
		...
		ctx.ScheduleAfter(func() {
			fx.SelectBacksideFilter(card.Player, ctx.Match, card.Player, match.SHIELDZONE,
				...
				ctx.Match.ReportActionInChat(card.Player, "Nastasha, Channeler of Suns destroyed to protect a shield.")
